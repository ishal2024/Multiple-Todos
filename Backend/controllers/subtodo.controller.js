...
async function getAllSubtodos(req, res) {
    try {
        ...
        const todo = await todoModel.findById(subtodoId)
        if (!todo) return res.status(400).json({ messgae: "Invalid TodoId" })
        const subtodos = await subtodosModel.find({ todo: subtodoId }).populate('owner')
...
async function updateSubtodo(req, res) {
    try {
        const subtodId = req.params?.subtodoId
        ...
        const updatedSubtodo = await subtodosModel.findByIdAndUpdate(subtodId,
            {
                $set: {
                    ...
                    description: req.body?.description ? req.body?.description : subtodo.description,
                    image : response?.url || subtodo?.image
                }
            }, { new: true }
        )

        res.status(200).json({ message: "Sub Todo is updated", updatedSubtodo })
