    ...
    async function onSubmit(data , e){
        ...
        try {
            ...
            if(response?.data?.userInfo){
                dispatch(userLogin(response?.data?.userInfo))
                setSpinner(false)
            ...
            <div className="w-full flex flex-col items-center justify-center">
                ...
                <form onSubmit={handleSubmit(onSubmit)}  className="md:w-96 w-80 flex flex-col items-center justify-center">
                    ...
                    <div className="flex items-center w-full bg-transparent border border-gray-300/60 h-12 rounded-full overflow-hidden pl-6 gap-2">
                        ...
                        <input {...register('email' , {required : true})} type="email" placeholder="Email id" className="bg-transparent text-gray-500/80 placeholder-gray-500/80 outline-none text-sm w-full h-full" required />                 
                    </div>
                    ...
                    <div className="flex items-center mt-6 w-full bg-transparent border border-gray-300/60 h-12 rounded-full overflow-hidden pl-6 gap-2">
                        ...
                        <input {...register('password' , {required : true})} type="password" placeholder="Password" className="bg-transparent text-gray-500/80 placeholder-gray-500/80 outline-none text-sm w-full h-full" required />
                    </div>
                    ...
                    <p className="text-gray-500/90 text-sm mt-4">Don’t have an account? <NavLink className="text-indigo-400 hover:underline" to={'/signup'}>Sign up</NavLink></p>
                </form>
            </div>
        </div>
  )
